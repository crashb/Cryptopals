# solution to http://cryptopals.com/sets/1/challenges/6
...
# breaks repeating XOR on bytearray.  takes ciphered bytes as argument along with number of key sizes to try.
# does not return anything
def breakRepeatingXOR(cipherBytes, numKeySizes):
	print("Breaking repeating XOR cipher...")
	keysizeNHDs = {}
	# find average normalized Hamming distance between first block and every other block
	for i in range(2, 41):
		numBlocks = int(math.ceil(len(cipherBytes)/i))
		firstBlock = cipherBytes[0:i]
		totalHammingDistance = 0
		for j in range(1, numBlocks):
			nextBlock = cipherBytes[i*j:i*(j+1)]
			if len(firstBlock) == len(nextBlock):
				totalHammingDistance += normalizedHammingDistance(firstBlock, nextBlock)
		keysizeNHDs[i] = totalHammingDistance / numBlocks
	# print("Normalized Hamming distances for key sizes: " + str(keysizeNHDs))
	
	# test the best numKeySizes keysizes
	for i in range(0, numKeySizes):
		keysize = min(keysizeNHDs, key=keysizeNHDs.get)
		keysizeNHD = keysizeNHDs[keysize]
		print("Trying key length " + str(keysize) + ", with normalized Hamming distance " + str(keysizeNHD))
		...
		keyString = ''
		for keyChar in keyBytes:
			keyString += chr(keyChar)
		print("Best key: " + keyString)
		plainBytes = repeatingKeyXOR(cipherBytes, keyBytes)
		print("Plaintext: " + plainBytes.decode("ascii"))
			
		# remove entry from keysizeNHDs dictionary so that we can try the next best keysize (if needed)
		del keysizeNHDs[keysize]
